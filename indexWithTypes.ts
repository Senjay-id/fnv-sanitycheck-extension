...
import { fs, log, selectors, util, types, actions } from 'vortex-api';
import crypto from 'crypto';
import { IEntry, RegGetValue, RegSetKeyValue } from 'winapi-bindings';
...
function isJIPLNVSEEnabled(api: types.IExtensionApi) {
  if (selectors?.getMod == null) {
    return false;
  }
  const mod: types.IMod | undefined = selectors?.getMod?.(api.getState(), FNV_SHORTNAME, JIP_LNVSE_MOD_ID);
  if (mod == null) {
    return false;
  }
...
function testASLR(api: types.IExtensionApi): Promise<types.ITestResult> {
  ...
  return new Promise<types.ITestResult>(async (resolve, reject) => {
    const res: types.ITestResult = {
      ...
      severity: 'warning',
      automaticFix: ((async () => disableASLR()) as any),
      onRecheck: ((async () => isASLREnabled()) as any)
    }
    return res !== undefined ? resolve(res) : resolve(undefined)
  });
}

function testAMDDriver(api: types.IExtensionApi): Promise<types.ITestResult> {
  const activeGameId = selectors.activeGameId(api.getState());
  if (activeGameId !== FNV_SHORTNAME) {
    return Promise.resolve(undefined);
  }
...
async function isExecutablePatched(api: types.IExtensionApi): Promise<boolean> {
...
function testExecutable(api: types.IExtensionApi): Promise<types.ITestResult> {
  return new Promise<types.ITestResult>(async (resolve) => {
    const activeGameId = selectors.activeGameId(api.getState());
    if (activeGameId !== FNV_SHORTNAME) {
    ...
    try {
      ...
      if (!isPatched || DEBUG) {
        const res: types.ITestResult = {
          ...
          automaticFix: (async () => downloadAndInstall4GBPatch(api)) as any,
          onRecheck: (async () => isExecutablePatched(api)) as any,
          severity: 'warning',
        };
        return resolve(res);
      }
...
function testTranslationPlugin(api: types.IExtensionApi): Promise<types.ITestResult> {
  return new Promise<types.ITestResult>(async (resolve) => {
    const activeGameId = selectors.activeGameId(api.getState());
    if (activeGameId !== FNV_SHORTNAME) {
      return resolve(undefined);
...
async function isLegacyNVSEExecutable(api: types.IExtensionApi): Promise<boolean> {
  const state = api.getState();
  const discovery = selectors.discoveryByGame(state, FNV_SHORTNAME);
  if (!discovery?.path) {
    return false;
...
function testLegacyNVSE(api: types.IExtensionApi): Promise<types.ITestResult> {
  return new Promise<types.ITestResult>(async (resolve) => {
    const activeGameId = selectors.activeGameId(api.getState());
    if (activeGameId !== FNV_SHORTNAME) {
      return resolve(undefined);
    ...
    try {
      ...
      if (isLegacy) {
        const res: types.ITestResult = {
          ...
          severity: 'warning',
          automaticFix: () => api.emitAndAwait('download-script-extender', FNV_SHORTNAME) as any,
          onRecheck: (async () => isLegacyNVSEExecutable(api)) as any,
        };
        return resolve(res);
      }
    } catch (err) {
...
function main(context: types.IExtensionContext) {
  ...
  context.registerTest('fnvsanitycheck-test-amd-driver', 'gamemode-activated', () => testAMDDriver(context.api) as any);
  context.registerTest('fnvsanitycheck-test-disable-aslr', 'gamemode-activated', () => testASLR(context.api) as any);
  context.registerTest('fnvsanitycheck-test-executable', 'gamemode-activated', () => testExecutable(context.api) as any);
  context.registerTest('fnvsanitycheck-test-translation-plugin', 'gamemode-activated', () => testTranslationPlugin(context.api) as any);
  context.registerTest('fnvsanitycheck-test-legacy-nvse', 'gamemode-activated', () => testLegacyNVSE(context.api) as any);
...
function disableASLR() {
  try {
    ...
    let mitigationOptionsBuffer: Buffer;
...
function isASLREnabled() {
  ...
  try {
    ...
    if (mitigationOptionsResult && Buffer.isBuffer(mitigationOptionsResult.value)) {
      const mitigationOptions = mitigationOptionsResult.value as Buffer;
...
function hasOutdatedAMDDriver() {
  try {
    ...
    if (driverVersionResult && typeof driverVersionResult.value === 'string') {
      const driverVersion = driverVersionResult.value as string;
...
async function createOverrideFiles(modPath, api: types.IExtensionApi) {
  let overrideCreated = false;
  try {
    await util.walk(modPath, util.toBlue(async () => async (entry: IEntry, stats: fs.Stats) => {
...
async function downloadAndInstall4GBPatch(api: types.IExtensionApi): Promise<void> {
  ...
  try {
    ...
    const modFiles: IFileInfo[] = await api.ext.nexusGetModFiles(FNV_SHORTNAME, patchModId);
